...
\end{equation}  This assumption is made to reduce the effect of over-fitting the model to data, and to allow the estimation procedure to converge by limiting the number of physiological parameters to estimate. This assumption also allows for the original model to be reduced into a simpler form with fewer synaptic connections shown in Figure~\ref{fig: Biological}. In order to simplify notation all excitatory temporal response curves will be characterised by $\alpha_{p}$ and $\tau_{p}$, slow inhibitory by $\alpha_{s}$ and $\tau_{s}$ and fast inhibitory by $\alpha_{f}$ and $\tau_{f}$.
